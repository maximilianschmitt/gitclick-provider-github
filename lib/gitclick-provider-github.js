...
var createTeamRepo = Promise.promisify(api.repos.createFromOrg);
var createRepository = Promise.promisify(api.repos.create);

var githubProvider = {
  createRepository: function(options, auth) {
    var create = options.team ? createTeamRepo : createRepository;

    auth.type = 'basic';
    api.authenticate(auth);
    options = parseOptions(options);

    return create(options).then(parseRepository).catch(handleError);
